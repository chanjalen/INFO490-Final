...
def results(request):
    ...
    try:
        ...
        pass  # guardrail: malformed JSON → use raw query only
    ...
    # ── 1. Session history (services.py) — always runs ───────────────────────
    record_search(
        request,
        query=query,
        genre=genre,
    ...
    # ── 2. DB history (models.py) — only when logged in ──────────────────────
    if request.user.is_authenticated:
        try:
            ...
            logger.warning("Failed to save DB search history: %s", exc)
